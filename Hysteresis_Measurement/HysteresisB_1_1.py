...
import tkinter.ttk # Second import necessary to overide basic tkinter widgets
...
from matplotlib.figure import Figure # Single graph
...
class HystGUI(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.hystGUI.title("Hysteresis B 1.1 - Jeffrey M. Ede")
        self.backClr = "#a1dbcd"
        self.btnClr = "#73c6b6"
        ...
        self.biases_in_meas = ('TotalV', 'ProbeV', 'BotElectV')
        ...
        # Load settings
        self.zero2VButton = tk.Button(self.hystGUI, text='0V and Bias', command=self.zero2V,
                                         bg=self.btnClr)
        # Load settings
        self.zero2VSaveButton = tk.Button(self.hystGUI, text='Save 0V and Bias', command=self.zero2VSave,
                                         bg=self.btnClr)
        ...
        tooltip.createToolTip(self.saveSettingsButton, "Save these settings to file.")
        tooltip.createToolTip(self.zero2VButton, "Plot measurements made under and after bias separately.\nMeasurements made after bias are labelled with that bias.")
        tooltip.createToolTip(self.loadSettingsButton, "Save measurements made under bias then save measurements after bias.\nMeasurements made after bias are labelled with that bias.")
        ...
        # zero2V
        self.zero2VButton.grid(row=9, column=7, sticky=tk.E+tk.W)
        
        # zero2V save
        self.zero2VSaveButton.grid(row=10, column=7, sticky=tk.E+tk.W)
    ...
    def bias_zero_samp(self, daq, device, probeAux, botElectAux, probeOffset, botElectOffset, demod_index,
        ...
        if biasTime != 0 and zeroVTime != 0.0:
            ...
            self.lengths = np.append(self.lengths, np.array([len(sample['t']), len(sample2['t']), probeOffset, botElectOffset]))
        ...
        if biasTime != 0:
            self.lengths = np.append(self.lengths, np.array([len(sample['t']), 0, probeOffset, botElectOffset]))
            return sample
        if zeroVTime != 0:
            self.lengths = np.append(self.lengths, np.array([0, len(sample2['t']), probeOffset, botElectOffset]))
            return sample2
    ...
    def plot(self):
        ...
        plot.title(x1+"-"+y1+" Plot - Jeffrey M. Ede")
        ...
        # Average over repeated values, if specified
        if self.avgRepeatedx.get() == 1:
            y, x = self.avg_xrepeats(y, x)
        if self.avgRepeatedy.get() == 1:
            x, y = self.avg_xrepeats(x, y)
    ...
    # Output full measurements dictionary
    def output_data(self):
        self.output_dictionary(self.measurements)
    
    # Output data dictionary
    def output_dictionary(self, measurements):        
        ...
        if x_get == 1 or self.avgRepeatedy.get() == 1:
            ...
            total = [0 for meas in measurements] # Sums of values to average over initially 0            
            start, end = 0, 1   # Average between y[start] and y[end] so x and y have same lengths
            prevx = measurements[x_axis][0]
    
            # If the next value of x is not the same as the previous, add data to the array
            for i in range(1, len(measurements[x_axis])):
                # If x is same as previous
                if measurements[x_axis][i] == prevx:
                    end += 1
                        
                    # Check if this is the last element of the array
                    if i == len(measurements[x_axis])-1:
                        # Add averages to output
                        for k, key in enumerate(measurements):
                            if key == x_axis:
                                text += str(prevx) + "    "
                            else:
                                for j in range(start, end):                              
                                    total[k] += measurements[key][i]
                ...
                # If x is not the same as previous
                    ...
                    for k, key in enumerate(measurements):
                        if key == x_axis:
                            text += str(prevx) + "    "
                            prevx = measurements[x_axis][i]
                        else:
                            for j in range(start, end):                              
                                total[k] += measurements[key][j]
                            text += str(total[k]/(end-start)) + "    "
                    text += "\n"
                    ...
                    if i == len(measurements[x_axis])-1:
                        for key in measurements:
                            text += str(measurements[key][i]) + "    "
                        text += "\n"
        
        else:
            # Print data to file without averaging repeats
            for i in range(len(measurements['X'])):
                for key in measurements:
                    text += str(measurements[key][i])+"    "
                text += "\n"
    ...
    # Creat 2 graphs showing 0V and bias measurements
    def zero2V(self):
        ...
        # Make sure that the there is only 1 bias to be graphed
        if (not x1 in self.biases_in_meas and not y1 in self.biases_in_meas) or (x1 in self.biases_in_meas and y1 in self.biases_in_meas):
            return
        
        # Give plot a title
        plot.title("Bias and After Bias - Jeffrey M. Ede")
        ...
        # Under Bias
        if x1 in self.biases_in_meas:
            x_bias, y_bias = self.zero2V_bias(y, x1)
        else:
            y_bias, x_bias = self.zero2V_bias(x, y1)
            
        # Average over repeated values, if specified
        if self.avgRepeatedx.get() == 1:
            y_bias, x_bias = self.avg_xrepeats(y_bias, x_bias)
        if self.avgRepeatedy.get() == 1:
            x_bias, y_bias = self.avg_xrepeats(x_bias, y_bias)
        
        ax[0].plot(x_bias, y_bias)
        ax[0].set_title(x1+"-"+y1+" Bias")
        
        # After bias
        if x1 in self.biases_in_meas:
            x_afterbias, y_afterbias = self.zero2V_afterbias(y, x1)
        else:
            y_afterbias, x_afterbias = self.zero2V_afterbias(x, y1)
            
        # Average over repeated values, if specified
        if self.avgRepeatedx.get() == 1:
            y_afterbias, x_afterbias = self.avg_xrepeats(y_afterbias, x_afterbias)
        if self.avgRepeatedy.get() == 1:
            x_afterbias, y_afterbias = self.avg_xrepeats(x_afterbias, y_afterbias)
            
        ax[1].plot(x_afterbias, y_afterbias)
        ax[1].set_title(x1+"-"+y1+" After Bias")
    ...
    def zero2VSave(self):
        ...
        # Measurements made under bias
        meas_bias = {'TotalV': np.empty((0)), 'X': np.empty((0)), 'Y': np.empty((0)), 'R': np.empty((0)), 'Phase': np.empty((0)),
                             't': np.empty((0)), 'ProbeV': np.empty((0)), 'BotElectV': np.empty((0))}
        
        trav_len = 0
        for i in range(0, len(self.lengths), 4):
            for key in meas_bias:
                if key == "TotalV":
                    bias = self.lengths[i+2]-self.lengths[i+3]
                    meas_bias[key] = np.append(meas_bias[key], np.full(int(self.lengths[i]), bias))
                    
                elif key == "ProbeV":
                    bias = self.lengths[i+2]
                    meas_bias[key] = np.append(meas_bias[key], np.full(int(self.lengths[i]), bias))
                
                elif key == "BotElectV":
                    bias = self.lengths[i+3]
                    meas_bias[key] = np.append(meas_bias[key], np.full(int(self.lengths[i]), bias))

                else:
                    meas_bias[key] = np.append(meas_bias[key], self.measurements[key][trav_len:(trav_len+int(self.lengths[i]))])
                
            trav_len += int(self.lengths[i]) + int(self.lengths[i+1])
        
        self.output_dictionary(meas_bias)
        
        # Measurements made after bias
        meas_bias = {'TotalV': np.empty((0)), 'X': np.empty((0)), 'Y': np.empty((0)), 'R': np.empty((0)), 'Phase': np.empty((0)),
                             't': np.empty((0)), 'ProbeV': np.empty((0)), 'BotElectV': np.empty((0))}
        ...
        for i in range(0, len(self.lengths), 4):
            for key in meas_bias:
                if key == "TotalV":
                    bias = self.lengths[i+2]-self.lengths[i+3]
                    meas_bias[key] = np.append(meas_bias[key], np.full(int(self.lengths[i+1]), bias))
                    ...
                    meas_bias[key] = np.append(meas_bias[key], self.measurements[key][(trav_len+int(self.lengths[i])):(trav_len+int(self.lengths[i])+int(self.lengths[i+1]))])
    ...
    def zero2V_bias(self, y, axis):
        
        x_bias = np.empty((0))
        y_bias = np.empty((0))
        
        # Use lengths of bias and 0V arrays 
        trav_len = 0
        for i in range(0, len(self.lengths), 4):
            if axis == "TotalV":
                bias = self.lengths[i+2]-self.lengths[i+3]
                x_bias = np.append(x_bias, np.full(int(self.lengths[i]), bias))
                y_bias = np.append(y_bias, y[trav_len:(trav_len+int(self.lengths[i]))])
    ...
    def zero2V_afterbias(self, y, axis):
        
        x_afterbias = np.empty((0))
        y_afterbias = np.empty((0))
        ...
        for i in range(0, len(self.lengths), 4):
            if axis == "TotalV":
                bias = self.lengths[i+2]-self.lengths[i+3]
                x_afterbias = np.append(x_afterbias, np.full(int(self.lengths[i+1]), bias))
                y_afterbias = np.append(y_afterbias, y[(trav_len+int(self.lengths[i])):(trav_len+int(self.lengths[i])+int(self.lengths[i+1]))])
        ...
        return x_afterbias, y_afterbias
