...
Wait times can be used to deliberately apply a voltage without taking measurements. This could useful when studying remnant effects of a bias as measurements taken under bias may be uninteresting.
            ...
            if(input$pattern=="Min-Max-Min"){
              ...
              if(input$pattern=="0-Max-0-Min-0"){
                ...
                for(i in 1:(as.integer((num_steps+1)/2)+addStep)){
                  # if(i == 1){
                  #   updateTextInput(session, "inText", value = paste("New text", l_idx))
                  # }
                  # After bias wait
